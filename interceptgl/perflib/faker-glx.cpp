...
//extern "C" void perf_end();

struct fd_pair *headerfd0;
...
FILE* getAndOpenLogFile(pid_t cur_pid){
     struct fd_pair *tmpfd = headerfd0;
	...
	const int *attrib_list, int *nelements)
{
	...
	*nelements = 0;
	...
	{
		configs = _glXChooseFBConfig(_dpy3D, DefaultScreen(_dpy3D), attrib_list,
			nelements);
		goto done;
	}
...
void glXSwapBuffers(Display *dpy, GLXDrawable drawable)
{
	TRY();
        ...
        if(cur_pid == 0){
            cur_pid = getpid();
            cur_tid = syscall(SYS_gettid);
            tmpFp = getAndOpenLogFile(cur_pid);
    	}
	...
	GLuint *maxBarriers)
{
	...
	return _glXQueryMaxSwapGroupsNV(_dpy3D, DefaultScreen(_dpy3D), maxGroups,
		maxBarriers);
